# Requirements Archive: v1.1 Courses Quality Polish
...
- [x] **COURSE-01**: PDF upload → course pipeline works end-to-end with reliable text extraction (positional sorting for multi-column layouts), proper chunking with heading detection, and AI structuring that produces usable courses
- [x] **COURSE-05**: Failed uploads and generation failures show clear, user-facing error messages — no silent `catch {}` blocks hiding failures
- [x] **COURSE-06**: Infrastructure fixes — server body size limit matches client limit (both at 10MB), dual storage (IndexedDB + SQLite) has consistent sync protocol
...
- [x] **COURSE-03**: Catalog has 8-10 useful seed courses available immediately on first launch, covering DSE prep (grammar, vocabulary, writing, reading strategy) and general English
- [x] **COURSE-07**: Course quality indicators visible in the catalog — badges show draft/reviewed/seed quality level so users can judge course maturity

### Analytics & Tracking

- [x] **COURSE-08**: Post-course tracking — system tracks whether user scores improve after completing courses (e.g., did grammar course reduce grammar errors in writing practice)
...
| COURSE-05 | Phase 7 | Complete |
| COURSE-06 | Phase 7 | Complete |
| COURSE-07 | Phase 9 | Complete |
| COURSE-08 | Phase 9 | Complete |
