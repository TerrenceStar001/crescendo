...
${stripped.slice(0, 6000)}`;
  ...
  if (genuineTruncation) {
    ...
    if (retryRaw) {
      ...
      console.warn(`[DSE] Passage retry: ${retryWc}w (min ${target.min}, grace ${minAccept})`);
      if (retryWc >= minAccept && retryWc <= target.max) {
...
${fragmentsSection}`;
  ...
  if (genuineTruncation) {
    ...
    if (retryRaw) {
      ...
      retryCleaned = retryCleaned.replace(/\n\*\*\*+\s*[\s\S]*$/, '');
      const retryWc = retryCleaned.split(/\s+/).filter(Boolean).length;
      console.warn(`[DSE] RAG passage retry: ${retryWc}w (min ${target.min}, grace ${minAccept})`);
      if (retryWc >= minAccept && retryWc <= target.max) {
        cleaned = retryCleaned;
        wc = retryWc;
      }
...
${difficulty === 'hard' ? `- MEDIUM/EASY — SUPPLEMENTARY:
  ...
  if (wasTruncated || wc < target.min || wc > target.max) {
    ...
    if (retryRaw) {
      ...
      retryCleaned = retryCleaned.replace(/\n(?:AI-generated|Generated by|Note:|Word count).*$/gi, '');
      const retryWc = retryCleaned.split(/\s+/).filter(Boolean).length;
      const minAcceptRetry = Math.floor(target.min * 0.9);
      console.warn(`[DSE] Pure AI passage retry: ${retryWc}w (min ${target.min}, grace ${minAcceptRetry})`);
      if (retryWc >= minAcceptRetry && retryWc <= target.max) {
        cleaned = retryCleaned;
        wasTruncated = false;
        wc = retryWc;
      }
